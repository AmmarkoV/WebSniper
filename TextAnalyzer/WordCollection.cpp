...
// HELPER FUNCTIONS
// *******************************************************************************
unsigned char IgnoreWord(unsigned char * theword , unsigned int wordsize )
{
  unsigned char retres = 1;
  if ( wordsize > 30 ) {} else
...
inline unsigned char NH_StringEqualToWordInCollection(word_collection * acol,unsigned int index,unsigned char * word,unsigned short length)
{
...
inline unsigned char H_StringEqualToWordInCollection(word_collection * acol,unsigned int index,unsigned long word_hash)
{
...
inline unsigned char CollectionOk(struct word_collection * acol)
{
  if ( acol != 0 )
    {
...
unsigned char DeleteWordCollection(struct word_collection * acol)
{
...
unsigned int FindWordAtCollection(word_collection * acol, unsigned long hash_data , unsigned char * word,unsigned short length)
{
  ...
  fprintf(stderr,"Searching for `%s` among %u words! ... ",word,acol->words_mem_length);
...
unsigned char AddNewWord2Collection(word_collection * acol,unsigned long hash_data ,unsigned char * word,unsigned short length)
{
  ...
  if (acol->words[current_memspot].string==0) { fprintf(stderr,"Could not allocate memory for Input word @ AddNewWord2Collection copy\n"); return 0; }
...
unsigned char AddWord2Collection( word_collection * acol,unsigned char * word,unsigned short length)
{
  if ( !CollectionOk(acol) ) return 0;
  ...
  unsigned char * act_word=0; // <- The actual word , upcased :P
  act_word = (unsigned char * ) malloc (sizeof( unsigned char ) * (length + 1 ) );
  if (act_word==0) { fprintf(stderr,"Could not allocate memory for Input word @ AddWord2Collection copy\n"); return 0; }
...
unsigned char DeleteWordFromCollection( word_collection * acol,unsigned char * word,unsigned short length)
{
  // TODO
...
unsigned int GetWordOccurances(word_collection * acol,unsigned char * word,unsigned short length)
{
  ...
  unsigned char * act_word=0; // <- The actual word , upcased :P
  act_word = (unsigned char * ) malloc (sizeof( unsigned char ) * (length + 1 ) );
  if (act_word==0) { fprintf(stderr,"Could not allocate memory for Input word @ GetWordOccurances copy\n"); return 0; }
  for ( unsigned short i=0; i<length; i++ )   act_word[i]=word[i];
  UpcaseIt(act_word,length);
